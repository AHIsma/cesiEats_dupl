...
export const OrderSummary = (props :any) => {
	...
	const [addressIds, setAddressIds] = useState<any[]>([]);
	...
	useEffect(() => {
		const fetchAllAddresses = async () => {
			try {
				const response = await CustomerService.getCustomerAdresses(customerId);
	...
	const orderItemsList = cart.cartItems.map((item :any) => (
		<Row>
			<Col xs={6} md={4}>
				<h5>{item.name}</h5>
			</Col>
